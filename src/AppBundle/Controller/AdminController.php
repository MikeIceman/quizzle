...
class AdminController extends Controller
{
    ...
    public function operationUpdateAction(Request $request)
    {
        ...
        if ($id && $action) {
            ...
            if ($action === 'accept' && $operation->getStatus() !== 'complete') {
                ...
                ($operation->getStatus() === 'complete' || $operation->getStatus() === 'pending')
            ) {
                // Reverse operation
                if ($operation->getStatus() === 'complete') {
                    ...
                    switch ($operation->getType()) {
                        ...
                        case 'exchange':
                            $operationUser->updateBalance($operation->getAmount());
                            $operationUser->updateBonusBalance(
                                -1 * $operation->getAmount() / $this->container->getParameter('loyalty_to_cash_rate')
                            );
                            break;
                    }
                }
    ...
    public function winningUpdateAction(Request $request)
    {
        ...
        if ($id && $action) {
            ...
            if ($action === 'send' && !in_array($prize->getStatus(), ['sent', 'received'])) {
